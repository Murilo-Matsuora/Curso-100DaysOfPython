...
class CafeForm(FlaskForm):
    ...
    location_url = StringField('Location URL', validators=[DataRequired(), URL(require_tld=True)])
...
def add_cafe():
    ...
    if form.validate_on_submit():
        print("True")
        new_cafe_data = [form.cafe.data, form.location_url.data, form.open.data, form.close.data, form.coffee.data, form.wifi.data, form.power.data]
        with open('cafe-data.csv', encoding='utf-8', mode='a', newline='') as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(new_cafe_data)
